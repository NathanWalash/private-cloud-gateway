    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-3xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            <h2 className="font-semibold text-text-primary">App Marketplace</h2>
            <p className="text-xs text-text-muted mt-0.5">{blueprints.length} apps available</p>
          </div>
          <button type="button" onClick={onClose} className="text-text-muted hover:text-text-primary p-1.5 rounded-lg hover:bg-white/5">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-6 py-3 border-b border-border shrink-0">
          <div className="relative">
            <MagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-text-muted" />
            <input
              autoFocus
              type="text"
          ...
          <div className="w-36 border-r border-border py-3 shrink-0 overflow-y-auto">
            <button
              ...
              className={`w-full text-left px-4 py-2 text-xs transition-colors ${
                !activeCategory ? 'text-accent font-medium bg-accent/10' : 'text-text-muted hover:text-text-primary hover:bg-white/5'
              }`}
            >
              All apps
            </button>
            {categories.map(cat => (
              <button
                ...
                className={`w-full text-left px-4 py-2 text-xs transition-colors flex items-center gap-2 capitalize ${
                  activeCategory === cat ? 'text-accent font-medium bg-accent/10' : 'text-text-muted hover:text-text-primary hover:bg-white/5'
                }`}
              >
          ...
          <div className="flex items-center justify-between">
            <div className="text-xs text-text-muted">
              {selected ? (() => {
                ...
                return (
                  <div>
                    <span>Installing: <strong className="text-text-primary">{bp?.name}</strong></span>
                    {bp?.depends_on && bp.depends_on.length > 0 && (
            ...
            <div className="flex gap-3">
              <button type="button" onClick={onClose} className="text-sm text-text-muted hover:text-text-primary px-4 py-2 transition-colors">
                Cancel
              </button>
              <button
                type="button"
